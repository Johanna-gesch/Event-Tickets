...
public class ETicketController implements Initializable {
    ...
    private void onSend(ActionEvent actionEvent) {
        try {
            ...
            if (email == null || email.isEmpty()) {
                displayError(new Exception("Please enter an email."));
                return;
            }
            ...
            if (!files.isEmpty()) {
                try {
                    Desktop.getDesktop().open(new File(System.getProperty("user.home") + "/Documents/TicketsToSend"));
                } catch (Exception ex) {
                    ex.printStackTrace();
                    displayError(new Exception("Couldn't open folder."));
                }
            }

            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setHeaderText("Almost done!");
            alert.setContentText("Drag the tickets from finder into the email and press send.");
            alert.showAndWait();

        } catch (Exception e) {
            e.printStackTrace();
            displayError(new Exception("Error preparing mail."));
        }
    }
    ...
    private List<File> saveTicketsAsImages(int numOfTickets) throws IOException {
        List<File> files = new ArrayList<>();

        // Create folder in finder
        String userHome = System.getProperty("user.home");
        File dir = new File(userHome + "/Documents/TicketsToSend");
        ...
        Scene tempScene = new Scene(container);

        WritableImage image = container.snapshot(new SnapshotParameters(), null);

        File file = new File(dir, "tickets_" + txtFName.getText() + ".png");
    ...
    private void openMail(String email) {
        try {
            String subject = String.valueOf(cboEventTickets.getValue()) + " Tickets";
            String body =
                    "Hey " + txtFName.getText() +
                    "\nHere are your tickets for the " + String.valueOf(cboEventTickets.getValue()) +
                    " event.\n\nBest Regards from\nSEA";
